...
import { FLEX_AUTH_OPTIONS, FLEX_AUTH_USER_OPTIONS, FLEX_AUTH_STRATEGIES, FLEX_AUTH_TOKENS } from '../auth.options';
import { FlexAuthService } from './auth.service';
import { strategiesFactory, optionsFactory } from '../auth.module';
import { FlexAuthResult } from './auth-result';
import { FlexTokenService } from './token/token.service';
import { FlexAuthSimpleToken, authCreateToken, FlexAuthJWTToken } from './token/token';
...
describe('auth-service', () => {
  ...
  const testToken = authCreateToken(FlexAuthSimpleToken, testTokenValue, ownerStrategyName);
  const invalidToken = authCreateToken(FlexAuthSimpleToken, testTokenValue, ownerStrategyName);
  const emptyToken = authCreateToken(FlexAuthSimpleToken, null, null);

  const failResult = new FlexAuthResult(false,
    resp401,
    null,
    ['Something went wrong.']);

  const successResult = new FlexAuthResult(true,
    resp200,
    '/',
    [],
  ...
  const successLogoutResult = new FlexAuthResult(true,
    resp200,
    '/',
    [],
    ['Successfully logged out.']);

  const successResetPasswordResult = new FlexAuthResult(true,
    resp200,
    '/',
    [],
    ['Successfully reset password.']);

  const successRequestPasswordResult = new FlexAuthResult(true,
    resp200,
    '/',
    [],
    ['Successfully requested password.']);

  const successRefreshTokenResult = new FlexAuthResult(true,
    resp200,
    null,
    [],
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [
        { provide: FLEX_AUTH_OPTIONS, useValue: {} },
  ...
  it('get test token before set', () => {
      ...
      authService.getToken().subscribe((val: FlexAuthSimpleToken) => {
  ...
  it('onTokenChange return correct stream and gets test token', (done) => {
      ...
      authService.onTokenChange()
        .pipe(first())
        .subscribe((token: FlexAuthSimpleToken) => {
          expect(spy).toHaveBeenCalled();
  ...
  it('authenticate failed', (done) => {
      ...
      authService.authenticate(ownerStrategyName).subscribe((authRes: FlexAuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('register failed', (done) => {
      ...
      authService.register(ownerStrategyName).subscribe((authRes: FlexAuthResult) => {
        expect(spy).toHaveBeenCalled();
        expect(authRes.isFailure()).toBeTruthy();
        expect(authRes.isSuccess()).toBeFalsy();
        expect(authRes.getMessages()).toEqual([]);
  ...
  it('logout failed', (done) => {
      ...
      authService.logout(ownerStrategyName).subscribe((authRes: FlexAuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('requestPassword failed', (done) => {
      ...
      authService.requestPassword(ownerStrategyName).subscribe((authRes: FlexAuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('resetPassword failed', (done) => {
      ...
      authService.resetPassword(ownerStrategyName).subscribe((authRes: FlexAuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('refreshToken failed', (done) => {
      ...
      authService.refreshToken(ownerStrategyName).subscribe((authRes: FlexAuthResult) => {
        expect(spy).toHaveBeenCalled();
        expect(authRes.isFailure()).toBeTruthy();
        expect(authRes.isSuccess()).toBeFalsy();
        expect(authRes.getMessages()).toEqual([]);
