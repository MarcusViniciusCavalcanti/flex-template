...
import { FlexMenuModule } from './menu.module';
import { FlexMenuBag, FlexMenuInternalService, FlexMenuItem, FlexMenuService } from './menu.service';
...
import { FlexMenuComponent } from './menu.component';

import { SpyLocation } from '@angular/common/testing';
import { FlexIconComponent } from '../icon/icon.component';
import { FlexIconLibraries } from '../icon/icon-libraries';
...
@Component({
  template: `<app-flex-menu [items]="items" [tag]="menuTag"></app-flex-menu>`,
})
export class SingleMenuTestComponent {
  @Input() items: FlexMenuItem[];
  @Input() menuTag: string;
  @ViewChild(FlexMenuComponent, { static: false }) menuComponent: FlexMenuComponent;

  constructor(public menuPublicService: FlexMenuService) {}
}

@Component({
  template: `
    <app-flex-menu [items]="firstMenuItems" [tag]="firstMenuTag"></app-flex-menu>
    <app-flex-menu [items]="secondMenuItems" [tag]="secondMenuTag"></app-flex-menu>
  `,
})
export class DoubleMenusTestComponent {
  @Input() firstMenuItems: FlexMenuItem[];
  @Input() secondMenuItems: FlexMenuItem[];
  @Input() firstMenuTag: string;
  @Input() secondMenuTag: string;
  @ViewChildren(FlexMenuComponent) menuComponent: QueryList<FlexMenuComponent>;
...
function createTestBed(routes: Routes = []) {
  TestBed.configureTestingModule({
    ...
    providers: [FlexMenuService],
  });

  TestBed.overrideProvider(Location, { useValue: new SpyLocationPathParameter() });

  const iconLibs: FlexIconLibraries = TestBed.get(FlexIconLibraries);
...
function createMenuItems(items: Partial<FlexMenuItem>[], menuInternaleService: FlexMenuInternalService): FlexMenuItem[] {
  menuInternaleService.prepareItems(items as FlexMenuItem[]);
  return items as FlexMenuItem[];
}

describe('FlexMenuItem', () => {

  it('should set tag attribute for menu services', () => {
    const { fixture } = createSingleMenuComponent([{ title: 'Home' }], 'menu');
    const menuTag = fixture.componentInstance.menuComponent.tag;
    expect(menuTag).toEqual('menu');
  });
  ...
  it('should change arrow direction when document direction changes', () => {
    ...
    const iconComponent = fixture.debugElement.query(By.directive(FlexIconComponent)) as DebugElement;
    const directionService: FlexLayoutDirectionService = TestBed.get(FlexLayoutDirectionService);

    expect(iconComponent.componentInstance.icon).toEqual('chevron-left-outline');

    directionService.setDirection(FlexLayoutDirection.RTL);
    fixture.detectChanges();
...
describe('menu services', () => {
  ...
  it('should operate with menu by tag', () => {
    ...
    const initialFirstMenuItemsCount = fixture.nativeElement
      .querySelector('app-flex-menu:first-child')
      .querySelectorAll('.menu-item')
      .length;
    const initialSecondMenuItemsCount = fixture.nativeElement
      .querySelector('app-flex-menu:last-child')
      .querySelectorAll('.menu-item')
      .length;
  ...
  it('should hide all expanded menu items', (done) => {
    ...
    menuService.onSubmenuToggle()
      ...
      .subscribe(([menuBagFirstCollapsed, menuBagSecondCollapsed]: FlexMenuBag[]) => {
...
describe('FlexMenuInternalService', () => {
  let router: Router;
  let menuInternalService: FlexMenuInternalService;
  ...
  beforeEach(() => {
    ...
    menuInternalService = TestBed.get(FlexMenuInternalService);
  });

  describe('selectFromUrl pathMatch full', () => {

    it('should select menu item with matching path', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
    ...
    it('should select menu item with matching path and fragment', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1', fragment: '1' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
    ...
    it('should select child menu item and its parent', (done) => {
      const items: Partial<FlexMenuItem>[] = [{
        link: '/menu-2',
        children: [{ link: '/menu-2/menu-2-level-2' }] as FlexMenuItem[],
      }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const parentMenuItem: FlexMenuItem = menuItems[0];
      const childMenuItem: FlexMenuItem = parentMenuItem.children[0];
    ...
    it('should select child menu item with fragment', (done) => {
      const items: Partial<FlexMenuItem>[] = [{
        link: '/menu-2',
        children: [{ link: '/menu-2/menu-2-level-2', fragment: '22' }] as FlexMenuItem[],
      }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const parentMenuItem: FlexMenuItem = menuItems[0];
      const childMenuItem: FlexMenuItem = parentMenuItem.children[0];
    ...
    it('should not select menu item with matching path if fragment doesn\'t match', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1', fragment: '1' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
    ...
    it('should not select menu item with matching fragment if path doesn\'t match', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1', fragment: '1' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
  ...
  describe('selectFromUrl pathMatch prefix', () => {
    it('should select menu item if url contains menu link', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1', pathMatch: 'prefix' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
    ...
    it('should select menu item if url contains menu link and fragment', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1', fragment: '1', pathMatch: 'prefix' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
    ...
    it('should not select menu item if url contains link without fragment', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1', fragment: '1', pathMatch: 'prefix' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
    ...
    it('should not select menu item if url contains fragment without link', (done) => {
      const items: Partial<FlexMenuItem>[] = [{ link: '/menu-1', fragment: '1', pathMatch: 'prefix' }];
      const menuItems: FlexMenuItem[] = createMenuItems(items, menuInternalService);
      const menuItem: FlexMenuItem = menuItems[0];
